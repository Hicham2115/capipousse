    ...
    <section id="a-propos" className="bg-white text-black">
      <div className="grid min-h-[37rem] lg:grid-cols-[minmax(0,0.83fr)_minmax(0,1.17fr)]">
        <div className="flex items-center bg-[#f8f7f4] px-7 py-16 sm:px-12 lg:px-16 xl:px-20">
          ...
          >
            <motion.p variants={fadeUpItem} className="text-sm font-semibold uppercase tracking-[0.34em]">Notre engagement</motion.p>
            <motion.h2 variants={fadeUpItem} className="mt-6 font-serif text-5xl leading-[0.92] tracking-[-0.05em] sm:text-6xl lg:text-[4.45rem]">Our<br />philosophy</motion.h2>
            <motion.span variants={fadeUpItem} className="mt-6 block h-px w-11 bg-black" />
            <motion.p variants={fadeUpItem} className="mt-7 text-base leading-[1.42] text-neutral-800 sm:text-lg">Chez Capi Pousse, nous croyons que la beauté commence par la nature. Notre mission est simple : offrir des soins capillaires et pour la barbe efficaces, sains et respectueux de votre corps et de l’environnement.</motion.p>
            <motion.p variants={fadeUpItem} className="mt-6 text-base font-semibold leading-6 sm:text-lg">Des formules pures, des résultats visibles, pour révéler le meilleur de vous-même.</motion.p>
      ...
      <motion.div
        ...
        variants={fadeUpContainer}
        className="mx-auto grid max-w-[1600px] gap-y-8 px-6 py-10 sm:grid-cols-2 sm:px-12 lg:grid-cols-4 lg:px-16"
      >
        {commitments.map(({ icon: Icon, title, copy }, index) => (
          <motion.div key={title} variants={fadeUpItem} className={`flex gap-5 px-2 lg:px-8 ${index > 0 ? "lg:border-l lg:border-neutral-300" : ""}`}>
