  ...
  return (
    <section className="relative bg-[#f8f8f6]">
      <div className="grid lg:grid-cols-[0.9fr_1.1fr]">
        <div className="px-7 pb-16 pt-10 sm:px-12 lg:px-16 lg:pb-28 lg:pt-10 xl:px-20">
          <p className="text-sm font-semibold uppercase tracking-[0.22em]">Pourquoi nous choisir ?</p>
          <h2 className="mt-3 font-serif text-5xl leading-none tracking-[-0.045em] sm:text-6xl lg:text-[4rem]">Why choose us</h2>
          <span className="mt-5 block h-px w-11 bg-black" />
          <p className="mt-5 max-w-xl text-base leading-6 sm:text-lg">Capi Pousse s’engage à vous offrir des soins capillaires et pour la barbe naturels, efficaces et sûrs.</p>
          <div className="mt-8 grid grid-cols-2 sm:grid-cols-3">
            {reasons.map(({ icon: Icon, title, text }, index) => (
              <article key={title} className={`min-h-48 px-4 py-4 text-center ${index % 3 !== 0 ? "sm:border-l sm:border-neutral-300" : ""} ${index > 2 ? "border-t border-neutral-300" : ""}`}>
      ...
      <div className="relative z-10 mx-auto -mt-4 grid max-w-5xl overflow-hidden rounded-xl border border-neutral-300 bg-white shadow-sm sm:grid-cols-3 lg:-mt-20">
        {guarantees.map(({ icon: Icon, title, text }, index) => (
          <div key={title} className={`flex items-center gap-4 px-7 py-6 ${index > 0 ? "border-t border-neutral-300 sm:border-l sm:border-t-0" : ""}`}>
